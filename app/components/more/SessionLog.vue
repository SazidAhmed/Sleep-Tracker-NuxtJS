...
// Virtualization
const shouldVirtualizeSessions = computed(
  () => filteredSessions.value.length > 40,
);
...
const estimatedSessionRowHeight = computed(() => {
  ...
  const heights = Object.values(sessionRowHeights.value).filter(
    (height) => height > 0,
  );
...
<template>
  <div class="space-y-4">
    ...
    <!-- Search & Filter Bar -->
    <div
      class="sticky top-0 z-20 space-y-3 bg-background/95 pb-4 backdrop-blur-sm"
    >
      <div class="relative">
        <Search
      ...
      <div
        ...
        :class="
          shouldVirtualizeSessions
            ? 'h-[70vh] overflow-y-auto pr-1 no-scrollbar'
            : ''
        "
      >
        <div
    ...
    <ConfirmationModal
      ...
      @cancel="
        ...
        itemRefs.get(sessionToDelete?.id ?? '')?.resetSwipe();
      "
    />
  </div>
</template>
