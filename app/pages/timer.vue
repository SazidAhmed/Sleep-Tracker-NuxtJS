...
const progressWidth = computed(
  () => `${Math.min(todaySummary.value.percentage, 100)}%`,
);
...
<template>
  <div class="min-h-screen p-4 pb-24">
    ...
    <header class="mb-6 flex items-center justify-between">
      ...
      <NuxtLink to="/" class="text-sm text-muted-foreground"> Done </NuxtLink>
    </header>
    ...
    <div class="mb-6 rounded-2xl border border-border/60 bg-card p-4">
      <div class="mb-2 flex items-center justify-between">
        ...
        <span class="text-sm font-semibold"
          >{{ Math.round(todaySummary.percentage) }}%</span
        >
      </div>
    ...
    <div class="rounded-3xl border border-border/60 bg-card p-5 shadow-sm">
      ...
      <div class="space-y-4">
        ...
        <p
          v-if="errorMessage && !activeSessionStart"
